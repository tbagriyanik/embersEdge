...
import { PlayerStats, Item, Entity, GameState, EntityType, TileType, GameSettings, Language, Projectile } from './types';
import { INITIAL_STATS, WORLD_SIZE, ITEMS, TIME_SCALE, RECIPES, TRANSLATIONS, TILE_WIDTH, TILE_HEIGHT, SAVE_KEY } from './constants';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const saveInterval = setInterval(() => {
      const { birds, ripples, ...stateToSave } = gameStateRef.current;
  ...
  const executeInteraction = useCallback((entityId: string) => {
    ...
    setGameState(prev => {
      ...
      const updatedEntities = prev.entities.map(e => {
        if (e.id === target.id) {
          ...
          return { 
            ...e, 
            health: newHealth,
            aiState: isHostile ? 'hunting' : 'fleeing',
            isFleeing: !isHostile
          } as Entity;
        }
        return e;
      ...
      if (newHealth <= 0) return handleEntityDeath(target, { ...prev, inventory: newInventory, entities: updatedEntities, playerStats: { ...prev.playerStats, equippedItemId: newEquippedId } });
  ...
  }, [showMessage, triggerRest, handleEntityDeath, isPaused]);

  const handleInteract = useCallback(() => {
    // Block gameplay interaction when paused
    if (isPaused) return;

    const state = gameStateRef.current;
    
    const nearestAnimal = state.entities.find((e: Entity) => 
      ['deer', 'rabbit', 'bear', 'scorpion'].includes(e.type) && 
      Math.sqrt((e.x - state.playerPos.x)**2 + (e.y - state.playerPos.y)**2) < 1.8
    );
    if (nearestAnimal) { executeInteraction(nearestAnimal.id); return; }

    const nearestObj = state.entities.find((e: any) => 
      !['deer', 'rabbit', 'bear', 'scorpion', 'road', 'bridge'].includes(e.type) &&
      Math.sqrt((e.x - state.playerPos.x)**2 + (e.y - state.playerPos.y)**2) < 1.6
    );
  ...
  const handleHUDAction = useCallback((action: 'use' | 'reorder' | 'equip' | 'repair', data: any) => {
  ...
  const update = useCallback((time: number) => {
    ...
    // Safety check for game start
    if (!gameStateRef.current.gameStarted) { 
      requestRef.current = requestAnimationFrame(update); 
      return; 
    }
    
    // FULL PAUSE LOGIC: Stop all state updates if menu is open
    if (isPaused) {
      requestRef.current = requestAnimationFrame(update); 
      return; 
    }
    ...
    setGameState(prev => {
      const now = Date.now();
      const nextProjectiles: Projectile[] = prev.projectiles
        .map(p => {
          const nextTrail = [...(p.trail || []), { x: p.x, y: p.y }].slice(-15);
          return { ...p, x: p.x + p.vx * dt, y: p.y + p.vy * dt, life: p.life - dt, trail: nextTrail };
        })
        .filter(p => p.life > 0);

      let playerHealthMod = 0;
      const nextEntities = prev.entities.map(ent => {
        if (['tree_oak', 'tree_pine', 'tree_palm', 'rock_standard', 'rock_iron', 'bush_berry', 'bush_flower', 'bush_dry', 'well', 'campfire', 'tent', 'workbench', 'hut', 'bridge', 'road', 'stone_wall', 'watchtower', 'castle_gate'].includes(ent.type)) {
          return ent;
        }
        ...
        let newX = ent.x;
        let newY = ent.y;
        let newState = ent.aiState || 'idle';
        let targetX = ent.targetX;
        let targetY = ent.targetY;
        let attackCooldown = (ent.attackCooldown || 0) - dt;

        if (ent.type === 'deer' || ent.type === 'rabbit') {
          if (distToPlayer < 5 || ent.isFleeing) {
            newState = 'fleeing';
            const edx = ent.x - prev.playerPos.x;
            const edy = ent.y - prev.playerPos.y;
            const emag = Math.sqrt(edx*edx + edy*edy);
            const speed = ent.type === 'rabbit' ? 6 : 4;
            newX += (edx/emag) * speed * dt;
            newY += (edy/emag) * speed * dt;
            if (distToPlayer > 12) ent.isFleeing = false;
          } else if (!targetX || now - (ent.lastAiTick || 0) > 4000) {
            if (Math.random() > 0.7) { newState = 'grazing'; targetX = ent.x; targetY = ent.y; }
            else { newState = 'idle'; targetX = ent.x + (Math.random() - 0.5) * 8; targetY = ent.y + (Math.random() - 0.5) * 8; }
            ent.lastAiTick = now;
          } else if (newState === 'idle' && targetX) {
            const tdx = targetX - ent.x; const tdy = targetY - ent.y; const td = Math.sqrt(tdx*tdx + tdy*tdy);
            if (td > 0.2) { newX += (tdx/td) * 1.5 * dt; newY += (tdy/td) * 1.5 * dt; }
          }
        }
        ...
        if (ent.type === 'bear' || ent.type === 'scorpion') {
          ...
          if (distToPlayer < huntRange) {
            if (distToPlayer < 1.3) {
              newState = 'attacking';
              if (attackCooldown <= 0) {
                if (now - prev.playerStats.lastDamageTime > PLAYER_INVINCIBILITY_MS) {
              ...
              newState = 'hunting';
              const hdx = prev.playerPos.x - ent.x; const hdy = prev.playerPos.y - ent.y; const hmag = Math.sqrt(hdx*hdx + hdy*hdy);
              const speed = ent.type === 'bear' ? 3.0 : 2.2; 
              newX += (hdx/hmag) * speed * dt; newY += (hdy/hmag) * speed * dt;
            }
          ...
          } else if (newState === 'prowling' && targetX) {
            const pdx = targetX - ent.x; const pdy = targetY - ent.y; const pd = Math.sqrt(pdx*pdx + pdy*pdy);
            if (pd > 0.2) { newX += (pdx/pd) * 1 * dt; newY += (pdy/pd) * 1 * dt; }
          }
        }

        newX = Math.max(2, Math.min(WORLD_SIZE - 2, newX));
        newY = Math.max(2, Math.min(WORLD_SIZE - 2, newY));
        return { ...ent, x: newX, y: newY, aiState: newState, targetX, targetY, attackCooldown } as Entity;
      });

      let finalState = { ...prev, entities: nextEntities, projectiles: nextProjectiles };
      nextProjectiles.forEach((p, pIdx) => {
        ...
        if (hitIdx > -1) {
          const target = nextEntities[hitIdx];
          const isHostile = ['bear', 'scorpion'].includes(target.type);
          target.health -= p.damage;
          target.aiState = isHostile ? 'hunting' : 'fleeing';
          target.isFleeing = !isHostile;
          p.life = 0;
          if (target.health <= 0) finalState = handleEntityDeath(target, finalState);
        }
      });
      ...
      const checkCollision = (px: number, py: number) => {
          if (getTileType(px, py) === 'water') return true;
          return updatedEntities.some((e: Entity) => !['rabbit', 'scorpion', 'deer', 'road', 'bridge'].includes(e.type) && Math.sqrt((e.x - px)**2 + (e.y - py)**2) < 0.55);
      };

      let finalX = prev.playerPos.x;
      let finalY = prev.playerPos.y;
      if (!checkCollision(finalX + moveX, finalY + moveY)) { finalX += moveX; finalY += moveY; }
      else { if (!checkCollision(finalX + moveX, finalY)) finalX += moveX; if (!checkCollision(finalX, finalY + moveY)) finalY += moveY; }
      ...
      const hungerDrain = 0.0007;
      const thirstDrain = 0.0010;
      if (prev.playerStats.hunger <= 0 || prev.playerStats.thirst <= 0) nextHealth -= 0.1;

      if (nextHealth <= 0) {
         setTimeout(() => setGameState((s: any) => ({ ...s, playerPos: { x: WORLD_SIZE/2, y: WORLD_SIZE/2 + 8 }, playerStats: INITIAL_STATS, inventory: [], gameStarted: false })), 100);
         return { ...finalState, playerStats: { ...prev.playerStats, health: 0 } };
      }
      ...
      return {
        ...
        entities: updatedEntities,
        playerStats: {
          ...finalState.playerStats,
          health: Math.min(prev.playerStats.maxHealth, nextHealth),
          lastDamageTime: lastDmgTime,
          isWalking: Math.sqrt(velocity.current.x**2 + velocity.current.y**2) > 0.4,
          hunger: Math.max(0, finalState.playerStats.hunger - hungerDrain),
          thirst: Math.max(0, finalState.playerStats.thirst - thirstDrain)
        },
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Still allow F, C and ESC to toggle menus even if paused
      ...
      if (key === 'escape') {
        if (uiState.craftingOpen || uiState.inventoryOpen || uiState.settingsOpen) {
          setUiState(prev => ({ ...prev, craftingOpen: false, inventoryOpen: false, settingsOpen: false }));
        } else {
      ...
      // Block all other keys if a menu is open
      if (isPaused) return;
      ...
      if (!isNaN(numKey) && numKey >= 1 && numKey <= 5) {
        ...
        const usableItems = currentInv.filter((i: Item) => ['tool', 'weapon', 'food'].includes(i.type));
        const item = usableItems[numKey - 1]; if (item) handleHUDAction('use', item);
        return;
      }
    ...
    const handleWheel = (e: WheelEvent) => {
      if (isPaused) return;
      e.preventDefault();
      setGameState(prev => ({ ...prev, viewConfig: { ...prev.viewConfig, zoom: Math.max(0.4, Math.min(2.5, prev.viewConfig.zoom + (e.deltaY > 0 ? -0.1 : 0.1))) } }));
    };
      ...
      if (e.button === 2 || e.button === 1) { isPanning.current = true; lastMousePos.current = { x: e.clientX, y: e.clientY }; e.preventDefault(); }
        ...
        if (state.playerStats.equippedItemId === 'bow') {
          ...
          if (arrowIdx > -1 && worldPos) {
            const adx = worldPos.x - state.playerPos.x;
            const ady = worldPos.y - state.playerPos.y;
            ...
            if (adist > 0.1) {
              ...
              setGameState(prev => {
                const ni = [...prev.inventory]; ni[arrowIdx].quantity -= 1;
                let newEquippedId = prev.playerStats.equippedItemId;
                const bowIdx = ni.findIndex(i => i.id === 'bow');
                if (bowIdx > -1) {
                  const bow = { ...ni[bowIdx] };
                  if (bow.durability !== undefined) {
                    bow.durability -= 1;
                    if (bow.durability <= 0) { ni.splice(bowIdx, 1); showMessage('broken'); newEquippedId = null; }
                    else ni[bowIdx] = bow;
                  }
                }
                const filteredInv = ni.filter(i => i.quantity > 0);
        ...
        if (worldPos) {
          const clickedEntity = gameStateRef.current?.entities.find((ent: any) => Math.sqrt((ent.x - worldPos.x)**2 + (ent.y - worldPos.y)**2) < 2.0);
          if (clickedEntity) {
            const mdist = Math.sqrt((clickedEntity.x - state.playerPos.x)**2 + (clickedEntity.y - state.playerPos.y)**2);
            if (mdist < 2.0) { executeInteraction(clickedEntity.id); }
            else { mouseTargetPos.current = { x: clickedEntity.x, y: clickedEntity.y }; targetEntityId.current = clickedEntity.id; }
          } else {
            const tile = getTileType(worldPos.x, worldPos.y);
            if (tile === 'water') { mouseTargetPos.current = worldPos; targetEntityId.current = 'water_point'; }
            else { mouseTargetPos.current = worldPos; targetEntityId.current = null; }
          }
        }
      }
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isPanning.current) {
        const pdx = e.clientX - lastMousePos.current.x; const pdy = e.clientY - lastMousePos.current.y;
        lastMousePos.current = { x: e.clientX, y: e.clientY };
        setGameState(prev => ({ ...prev, viewConfig: { ...prev.viewConfig, cameraOffsetX: prev.viewConfig.cameraOffsetX + pdx, cameraOffsetY: prev.viewConfig.cameraOffsetY + pdy } }));
      }
    };
    ...
    window.addEventListener('mousedown', handleMouseDown); window.addEventListener('mousemove', handleMouseMove); window.addEventListener('mouseup', handleMouseUp);
    window.addEventListener('contextmenu', handleContextMenu);
    return () => {
      window.removeEventListener('keydown', handleKeyDown); window.removeEventListener('keyup', handleKeyUp);
      window.removeEventListener('wheel', handleWheel);
      window.removeEventListener('mousedown', handleMouseDown); window.removeEventListener('mousemove', handleMouseMove); window.removeEventListener('mouseup', handleMouseUp);
  ...
  if (!gameState.gameStarted) {
    return (
      <MainMenu 
        ...
        onStart={() => { SoundManager.init(); SoundManager.startForestAmbience(); setGameState(prev => ({ ...prev, gameStarted: true, entities: spawnEntities(8000), inventory: [], playerPos: { x: WORLD_SIZE / 2, y: WORLD_SIZE / 2 + 8 }, playerStats: INITIAL_STATS })); }}
        onContinue={() => { const saved = localStorage.getItem(SAVE_KEY); if (saved) { try { const loadedState = JSON.parse(saved); setGameState({ ...loadedState, gameStarted: true, birds: [], ripples: [] }); SoundManager.init(); SoundManager.startForestAmbience(); } catch(e) { showMessage("Failed to load game save.", true); } } }}
        settings={gameState.settings} onUpdateSettings={s => setGameState(prev => ({ ...prev, settings: s }))}
        playerStats={gameState.playerStats} onUpdatePlayerStats={ps => setGameState(prev => ({ ...prev, playerStats: ps }))}
      />
    );
  }
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-stone-950">
      ...
      {isPaused && (
        <div className="absolute inset-0 z-40 bg-black/10 backdrop-blur-[1px] pointer-events-none flex items-start justify-center pt-24">
          <div className="px-8 py-3 bg-stone-900/80 border border-white/10 rounded-full shadow-2xl">
            <span className="text-white/60 font-black text-xs uppercase tracking-[0.5em] animate-pulse">Game Paused</span>
          </div>
        </div>
      )}
      ...
      {isResting && (
        <div className="absolute inset-0 z-40 bg-black/40 backdrop-blur-[2px] pointer-events-none flex items-center justify-center">
          <div className="text-white font-black text-4xl animate-pulse flex flex-col items-center">
            <span>💤</span> <span className="text-sm mt-2 opacity-50 tracking-[0.3em] uppercase">{t('rest_tent')}</span>
          </div>
        </div>
      )}
      
      <HUD 
        stats={gameState.playerStats} time={gameState.time} message={uiState.message} gameState={gameState}
        onAction={handleHUDAction} onZoom={() => {}} onRotate={() => {}} 
      ...
      {uiState.craftingOpen && <Crafting inventory={gameState.inventory} playerLevel={gameState.playerStats.level} isNearWorkbench={isNearWorkbench} onCraft={(recipeId) => {
          ...
          setGameState((prev: any) => {
             ...
             Object.entries(recipe.ingredients).forEach(([id, qty]) => { const idx = ni.findIndex(i => i.id === id); if (idx > -1) { ni[idx].quantity -= (qty as number); if (ni[idx].quantity <= 0) ni.splice(idx, 1); } });
             const newItem = { ...recipe.output };
             let updatedEntities = [...prev.entities];
             let placedDirectly = false;
             if (newItem.type === 'structure') {
                const spot = findPlacementSpot(prev.playerPos.x, prev.playerPos.y, updatedEntities);
                if (spot) { 
                  updatedEntities.push({ id: `struct-${Date.now()}`, x: spot.x, y: spot.y, type: newItem.id as EntityType, health: 10, maxHealth: 10, spawnTime: Date.now() }); 
                  placedDirectly = true; showMessage(t('placed') + ': ' + recipe.name, true); SoundManager.playUI('fanfare'); 
                }
             }
             if (!placedDirectly) { const existingIdx = ni.findIndex(i => i.id === newItem.id); if (existingIdx > -1) ni[existingIdx].quantity += newItem.quantity; else ni.push(newItem); SoundManager.playUI('fanfare'); showMessage(t('crafted') + ': ' + recipe.name, true); }
      ...
      {uiState.settingsOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
           <div className="w-full max-w-md bg-stone-900 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-white">
              <h2 className="text-3xl font-black mb-8 tracking-tighter text-amber-500 uppercase">{t('settings')}</h2>
              <div className="flex flex-col gap-4">
                <button onClick={() => setGameState(prev => ({...prev, settings: {...prev.settings, soundEnabled: !prev.settings.soundEnabled}}))} className={`py-3.5 rounded-xl font-black text-[11px] border ${gameState.settings.soundEnabled ? 'bg-amber-500 text-stone-900 border-amber-500' : 'bg-white/5 border-white/10 text-white/50'}`}>{t('sound')}: {gameState.settings.soundEnabled ? 'ON' : 'OFF'}</button>
                <div className="flex flex-col gap-3 mt-4">
                   <span className="text-[11px] font-black tracking-widest text-white/40 uppercase">{t('language')}</span>
                   <div className="grid grid-cols-2 gap-3">{(['en', 'tr'] as Language[]).map(l => (
              ...
              <button onClick={() => setUiState(s => ({...s, settingsOpen: false}))} className="w-full py-4 mt-10 bg-white text-stone-950 font-black rounded-xl uppercase tracking-widest text-[11px] hover:bg-amber-500 transition-colors shadow-lg active:scale-95">{t('back')}</button>
           </div>
        </div>
      )}
    </div>
