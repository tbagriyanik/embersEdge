...
export const getTileType = (x: number, y: number, level: number = 1): TileType => {
  ...
  const n2 = Math.sin(x * 0.8 + 2) * Math.sin(y * 0.6 - 1);
  const n3 = Math.cos(x * 1.5) * Math.sin(y * 1.5);
...
const App: React.FC = () => {
  ...
  const spawnEntities = useCallback((count: number, level: number, specificType?: EntityType[]) => {
    ...
    const types: EntityType[] = specificType || ['tree', 'rock', 'tree', 'rock', 'bush', 'deer', 'rabbit'];
    let spawned = 0;
    while (spawned < count) {
      ...
      if (tile !== 'water') {
        // More bias for trees and rocks on grass
        let type = types[Math.floor(Math.random() * types.length)];
        if (tile === 'grass' && Math.random() < 0.4) {
            type = Math.random() > 0.5 ? 'tree' : 'rock';
        }
        
        newEnts.push({
          id: `ent-${Date.now()}-${spawned}-${Math.random()}`, x: rx, y: ry, type,
          health: ['deer', 'bear'].includes(type) ? 8 : type === 'rabbit' ? 2 : 5,
          maxHealth: ['deer', 'bear'].includes(type) ? 8 : type === 'rabbit' ? 2 : 5,
          targetX: rx,
          targetY: ry,
          spawnTime: Date.now()
        });
  ...
  const interact = useCallback(() => {
    setGameState(prev => {
      ...
      if (ent.type === 'well') { SoundManager.playGather('bush'); return { ...prev, playerStats: { ...newStats, thirst: Math.min(100, newStats.thirst + 40) } }; }
      if (ent.type === 'tent' || ent.type === 'hut') { SoundManager.playUI('click'); showMessage('Resting...', true); return { ...prev, time: (prev.time + 600) % 2400, playerStats: { ...newStats, health: Math.min(100, newStats.health + 25), stamina: 100 } }; }
      ...
      if (updatedHealth <= 0) {
        ...
        let item = null;
        if (ent.type === 'tree') item = { ...ITEMS.wood, quantity: 3 };
        else if (ent.type === 'rock') item = { ...ITEMS.stone, quantity: 2 };
        else if (['deer', 'bear', 'rabbit', 'scorpion', 'bush'].includes(ent.type)) item = ent.type === 'bush' ? { ...ITEMS.berry, quantity: 2 } : { ...ITEMS.meat_raw, quantity: 2 };
  ...
  }, [interact, uiState, gameState.gameStarted, gameState.inventory, handleInventoryAction]);
  ...
  const tick = useCallback(() => {
    if (!gameState.gameStarted || gameOver) return;
    ...
    setGameState(prev => {
      ...
      if (mag > 0.01) {
        ...
        if (footstepAccumulator.current > 0.3) {
          footstepAccumulator.current = 0;
          SoundManager.playFootstep(getTileType(nextX, nextY), mag * 20);
        }
      }
      ...
      return {
        ...prev, playerPos: { x: nextX, y: nextY },
        time: newTime, isDay: newTime > 400 && newTime < 2000,
        entities: finalEntities,
        playerStats: {
          ...prev.playerStats, isWalking: mag > 0.001, facing: newFacing,
  ...
  }, [gameState.gameStarted, gameOver, interact, spawnEntities]);
