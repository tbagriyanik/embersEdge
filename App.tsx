...
import { PlayerStats, Item, Entity, GameState, EntityType, WeatherType, TileType, FacingDirection, GameSettings, Language, Projectile } from './types';
...
export const getTileType = (x: number, y: number, level: number = 1): TileType => {
  ...
  if (distToCenter < 10) return 'water';
  if (distToCenter < 15) return 'sand';

  const n1 = Math.sin(x * 0.2) * Math.cos(y * 0.2);
  const n2 = Math.sin(x * 0.1 + y * 0.1) * 0.5;
  const combinedNoise = n1 * 0.6 + n2 * 0.4;

  if (combinedNoise > 0.75) return 'water';
  if (combinedNoise > 0.55) return 'sand';
  return 'grass';
};

const App: React.FC = () => {
  const [gameState, setGameState] = useState<GameState>({
    playerPos: { x: WORLD_SIZE / 2, y: WORLD_SIZE / 2 + 15 },
  ...
  const addXP = useCallback((amount: number) => {
    setGameState(prev => {
      ...
      let leveledUp = false;
      const xpToNext = 100 * Math.pow(newLevel, 1.2);
      if (newXP >= xpToNext) {
        newXP -= xpToNext;
      ...
      if (leveledUp) {
        ...
        const unlocked = RECIPES.filter(r => r.levelRequired === newLevel).map(r => r.name).join(", ");
        showMessage(`${t('level_up')} ${newLevel}${unlocked ? `\n🎁 Unlocked: ${unlocked}` : ""}`, true);
      }
      return {
        ...prev,
        playerStats: {
          ...
          maxHealth: leveledUp ? prev.playerStats.maxHealth + 5 : prev.playerStats.maxHealth,
          health: leveledUp ? prev.playerStats.maxHealth + 5 : prev.playerStats.health,
        }
      };
    });
  }, [showMessage, t]);
    ...
    setGameState(prev => {
      ...
      if (action === 'use') {
        ...
        if (item.type === 'food' && item.effect) {
            ...
            ns.health = Math.min(ns.maxHealth, ns.health + (item.effect.health || 0));
  ...
  const spawnEntities = useCallback((count: number, level: number) => {
    ...
    while (spawned < count) {
      ...
      if (tile !== 'water') {
        ...
        newEnts.push({
          ...
          health: ['deer', 'bear'].includes(finalType) ? 12 : (finalType === 'rabbit' ? 3 : 8),
          maxHealth: ['deer', 'bear'].includes(finalType) ? 12 : (finalType === 'rabbit' ? 3 : 8),
          targetX: rx,
          targetY: ry,
          spawnTime: Date.now()
        });
  ...
  const triggerRest = useCallback(() => {
    ...
    setTimeout(() => {
      setGameState(prev => ({
        ...
        playerStats: {
          ...prev.playerStats,
          health: Math.min(prev.playerStats.maxHealth, prev.playerStats.health + 30),
          stamina: prev.playerStats.maxStamina
        }
      }));
      showMessage('Rested for several hours', true);
    }, 1000);
    setTimeout(() => { setIsResting(false); }, 2500);
  ...
  const interact = useCallback(() => {
    ...
    setGameState(prev => {
      ...
      const equipped = prev.inventory.find(i => i.id === prev.playerStats.equippedItemId);
      const isBow = equipped?.id === 'bow';
      const range = isBow ? 8 : 1.8;
      
      const newStats = { ...prev.playerStats, lastInteractTime: Date.now() };

      // Bow Mechanic
      if (isBow) {
        const arrowIdx = prev.inventory.findIndex(i => i.id === 'arrow');
        if (arrowIdx === -1 || prev.inventory[arrowIdx].quantity <= 0) {
           showMessage('No arrows left!', true);
           return { ...prev, playerStats: newStats };
        }
        
        // Find nearest entity in facing direction or range
        const idx = prev.entities.findIndex(e => Math.sqrt((e.x - px)**2 + (e.y - py)**2) <= range);
        
        // Spawn projectile
        const dx = prev.playerStats.facing.includes('e') ? 1 : (prev.playerStats.facing.includes('w') ? -1 : 0);
        const dy = prev.playerStats.facing.includes('s') ? 1 : (prev.playerStats.facing.includes('n') ? -1 : 0);
        const projectile: Projectile = {
          id: `proj-${Date.now()}`,
          x: px, y: py,
          vx: dx * 0.25, vy: dy * 0.25,
          damage: equipped?.effect?.damage || 8,
          ownerId: 'player',
          life: 40
        };

        const newInv = [...prev.inventory];
        newInv[arrowIdx].quantity -= 1;
        if (newInv[arrowIdx].quantity <= 0) newInv.splice(arrowIdx, 1);
        
        SoundManager.playGather('bush_dry'); // Bow sound
        return { ...prev, projectiles: [...prev.projectiles, projectile], inventory: newInv, playerStats: { ...newStats, stamina: Math.max(0, newStats.stamina - 2) } };
      }
      ...
      if (ent.type === 'tent' || ent.type === 'hut') { triggerRest(); return { ...prev, playerStats: newStats }; }
      if (ent.type === 'workbench') { setUiState(u => ({ ...u, craftingOpen: true })); return { ...prev, playerStats: newStats }; }

      SoundManager.playGather(ent.type);
      
      const damageBonus = equipped?.effect?.damage || (prev.playerStats.equippedItemId === 'axe' ? 3 : 1);
      const updatedHealth = ent.health - damageBonus;

      if (updatedHealth <= 0) {
        newEnts.splice(idx, 1);
        let item = null;
        if (ent.type.startsWith('tree')) item = { ...ITEMS.wood, quantity: 3 };
        else if (ent.type.startsWith('rock')) item = { ...ent.type === 'rock_iron' ? ITEMS.iron : ITEMS.stone, quantity: 2 };
      ...
      return { ...prev, entities: newEnts, inventory: newInv, playerStats: { ...newStats, stamina: Math.max(0, newStats.stamina - 2) } };
    });
  }, [addXP, triggerRest, showMessage]);
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (key === 'f') { setUiState(u => ({...u, inventoryOpen: !u.inventoryOpen, craftingOpen: false, settingsOpen: false})); e.preventDefault(); return; }
    if (key === 'c') { setUiState(u => ({...u, craftingOpen: !u.craftingOpen, inventoryOpen: false, settingsOpen: false})); e.preventDefault(); return; }
    if (uiState.craftingOpen || uiState.inventoryOpen || uiState.settingsOpen) { if (key === 'escape') setUiState(p => ({...p, craftingOpen: false, inventoryOpen: false, settingsOpen: false})); return; }
  ...
  }, [interact, uiState, gameState.inventory, handleInventoryAction, isResting]);
  ...
  const handleKeyUp = useCallback((e: KeyboardEvent) => {
    ...
    if (['w', 'a', 's', 'd', 'arrowup', 'arrowleft', 'arrowdown', 'arrowright'].includes(key)) {
        ...
        if (activeKeys.current.has('d') || activeKeys.current.has('arrowright')) dx = 1;
        inputDir.current = { x: dx, y: dy };
    } else if (key === 'e') { isInteracting.current = false; }
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    setGameState(prev => {
      ...
      let newProjectiles = prev.projectiles.map(p => ({ ...p, x: p.x + p.vx, y: p.y + p.vy, life: p.life - 1 })).filter(p => p.life > 0);
      let newEnts = [...prev.entities];
      newProjectiles = newProjectiles.filter(p => {
         const hitIdx = newEnts.findIndex(e => Math.sqrt((e.x - p.x)**2 + (e.y - p.y)**2) < 1.0 && !['road', 'bridge'].includes(e.type));
         if (hitIdx > -1) {
            const ent = newEnts[hitIdx];
            ent.health -= p.damage;
            if (ent.health <= 0) {
               newEnts.splice(hitIdx, 1);
               addXP(15);
            }
            return false;
         }
      ...
      const isOverBridge = prev.entities.some(e => e.type === 'bridge' && Math.floor(e.x) === Math.floor(nextX) && Math.floor(e.y) === Math.floor(nextY));
      const canMove = targetTile !== 'water' || isOverBridge;
      ...
      if (mag > 0.01) {
        ...
        if (Math.abs(cx) > Math.abs(cy)) newFacing = cx > 0 ? 'se' : 'sw';
        else newFacing = cy > 0 ? 'se' : 'ne';
      ...
      if (spawnTimerRef.current > 60) {
        ...
        const autoEnts = spawnEntities(10, prev.playerStats.level);
        newEnts.push(...autoEnts);
      }
      ...
      return {
        ...
        playerStats: {
          ...
          hunger: Math.max(0, prev.playerStats.hunger - 0.005),
          thirst: Math.max(0, prev.playerStats.thirst - 0.008),
          health: Math.max(0, prev.playerStats.health + (prev.playerStats.hunger <= 0 ? -0.05 : 0.01))
        }
      };
    });
    requestRef.current = requestAnimationFrame(tick);
  }, [gameState.gameStarted, gameOver, interact, spawnEntities, isResting, addXP]);
  ...
  useEffect(() => {
    const initialEnts = spawnEntities(450, 1);
  ...
  return (
    <div 
      ...
      onWheel={e => setGameState(p => ({...p, viewConfig: {...p.viewConfig, zoom: Math.max(0.5, Math.min(2.0, p.viewConfig.zoom - e.deltaY * 0.001))}}))}
      onPointerDown={e => {
        if (e.button === 0 && !(e.target as HTMLElement).closest('.pointer-events-auto') && gameState.gameStarted) {
            setIsPanning(true); lastMouseClientPos.current = { x: e.clientX, y: e.clientY };
      ...
      onPointerMove={e => {
        ...
        setGameState(p => ({...p, viewConfig: {...p.viewConfig, cameraOffsetX: p.viewConfig.cameraOffsetX + dx, cameraOffsetY: p.viewConfig.cameraOffsetY + dy}}));
      }}
      onPointerUp={e => { setIsPanning(false); (e.target as HTMLElement).releasePointerCapture(e.pointerId); }}
    >
      <GameCanvas gameState={gameState} />
      <div className={`fixed inset-0 z-[150] pointer-events-none transition-opacity duration-1000 ${isCritical ? 'opacity-100' : 'opacity-0'}`} style={{background: 'radial-gradient(circle, transparent 40%, rgba(220, 38, 38, 0.4) 100%)'}}>
        <div className={`absolute inset-0 bg-red-600/10 ${isCritical ? 'animate-vitals-pulse' : ''}`} />
      </div>
      <div className={`fixed inset-0 bg-black z-[200] pointer-events-none transition-opacity duration-1000 flex flex-col items-center justify-center ${isResting ? 'opacity-100' : 'opacity-0'}`}><div className="text-white font-black text-4xl tracking-widest animate-pulse uppercase">Resting...</div></div>
      ...
      <div className="fixed bottom-6 right-6 flex flex-col items-center gap-4 z-50 pointer-events-auto">
         <button onClick={() => interact()} className="w-16 h-16 bg-amber-500 rounded-full flex items-center justify-center text-3xl font-black shadow-xl active:scale-90 transition-all">E</button>
      ...
      {uiState.craftingOpen && <Crafting inventory={gameState.inventory} playerLevel={gameState.playerStats.level} isNearWorkbench={gameState.entities.some(e => e.type === 'workbench' && Math.sqrt((e.x-gameState.playerPos.x)**2 + (e.y-gameState.playerPos.y)**2) < 2)} onCraft={recipeId => {
        ...
        setGameState(prev => {
          ...
          if (recipe.output.type === 'structure') {
             const newEntity: Entity = { id: `struct-${Date.now()}`, x: prev.playerPos.x, y: prev.playerPos.y, type: recipe.output.id as EntityType, health: 50, maxHealth: 50 };
      ...
      <style>{`
        @keyframes vitals-pulse { 0%, 100% { opacity: 0.2; } 50% { opacity: 0.6; } }
        .animate-vitals-pulse { animation: vitals-pulse 2s ease-in-out infinite; }
      `}</style>
    </div>
  );
};
